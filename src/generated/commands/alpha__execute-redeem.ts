...
export const command = 'execute-redeem'
export const describe = "Execute a sell order to redeem on-chain tokens for USDT/USDC, returns order number"
...
export const jsonSchema = {
  ...
  properties: {
    'from-token-code': {
      type: 'string',
      description: "On-chain token to sell (`DEX_<id>`). Must match the quote request.",
      
    },
    'from-token-amount': {
      type: 'string',
      description: "Amount to sell (positive decimal as string). Must match the quote request.",
      
    },
    'to-token-code': {
      type: 'string',
      description: "Payment token to receive (`CEX_<id>`, e.g. `CEX_1` for USDT). Must match the quote request.",
...
export const builder = (yargs: any) => yargs
  .option('from-token-code', { type: 'string', demandOption: true, describe: "On-chain token to sell (`DEX_<id>`). Must match the quote request." })
  .option('from-token-amount', { type: 'string', demandOption: true, describe: "Amount to sell (positive decimal as string). Must match the quote request." })
  .option('to-token-code', { type: 'string', demandOption: true, describe: "Payment token to receive (`CEX_<id>`, e.g. `CEX_1` for USDT). Must match the quote request." })
...
export const handler = async (argv: any) => {
  ...
  checkConfirm(argv, {
    operation: 'alpha execute-redeem',
    method: 'POST',
    path: '/v5/alpha/trade/redeem',
    params: argv,
  })
