...
export const command = 'account-no-convert-repay'
export const describe = "No-Convert Repay (Lossless Repay)"
...
export const jsonSchema = {
  ...
  properties: {
    'coin': {
      type: 'string',
      description: "The coin with liability. Coin name, uppercase only.\nIf not passed, all liabilities will be repaid using available spot balances.\n",
      
    },
    'amount': {
      type: 'string',
      description: "Repay amount.\n- If `coin` is not passed, `amount` cannot be passed\n- If `coin` is passed but `amount` is not passed, the system uses the full available spot balance of the debt currency\n",
...
export const builder = (yargs: any) => yargs
  .option('coin', { type: 'string', describe: "The coin with liability. Coin name, uppercase only.\nIf not passed, all liabilities will be repaid using available spot balances.\n" })
...
export const handler = async (argv: any) => {
  ...
  checkConfirm(argv, {
    operation: 'account account-no-convert-repay',
    method: 'POST',
    path: '/v5/account/no-convert-repay',
    params: argv,
  })
