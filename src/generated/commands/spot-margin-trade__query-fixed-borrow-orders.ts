...
export const command = 'query-fixed-borrow-orders'
export const describe = "Query Fixed-Rate Borrow Orders"
...
export const jsonSchema = {
  ...
  properties: {
    ...
    'state': {
      type: 'string',
      description: "Filter by order state.\n- `1`: Matching (order accepted, matching in progress)\n- `2`: Partially filled & cancelled\n- `3`: Fully filled\n- `4`: Cancelled\n",
      enum: ['1', '2', '3', '4'],
    },
    'term': {
      type: 'string',
...
export const builder = (yargs: any) => yargs
  ...
  .option('state', { type: 'string', choices: ['1', '2', '3', '4'], describe: "Filter by order state.\n- `1`: Matching (order accepted, matching in progress)\n- `2`: Partially filled & cancelled\n- `3`: Fully filled\n- `4`: Cancelled\n" })
...
const innerHandler = createHandler({
  ...
  mapArgs: (argv) => ({
    method: 'GET',
    path: '/v5/spot-margin-trade/fixedborrow-order-info',
    query: filterDefined({ orderId: argv['order-id'], orderCurrency: argv['order-currency'], state: argv['state'], term: argv['term'], limit: argv['limit'], cursor: argv['cursor'] }),
